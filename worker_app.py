...
def _is_worker_authorized() -> bool:
    if not WORKER_API_KEY:
        return True
    incoming = (request.headers.get("X-Worker-Key") or "").strip()
    return incoming == WORKER_API_KEY


@app.post("/api/worker/download")
def api_worker_download():
    if not _is_worker_authorized():
        return jsonify({"ok": False, "error": "Unauthorized worker request."}), 401
    ...
    raw_url = (body.get("url") or request.form.get("url") or "").strip()
    ...
    shutil.rmtree(job_dir, ignore_errors=True)
    return error_response(
        "BLOB_READ_WRITE_TOKEN is not configured on worker.",
        500,
    )
